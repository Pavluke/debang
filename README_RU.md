...
**Примечание по интеграции с IDE**: Из-за известного ограничения в Dart Language
Server (см. [issue 38894](https://github.com/dart-lang/sdk/issues/38894)), при
вызове `.debang()` на nullable-значении быстрые исправления в IDE (например, в
VS Code или Android Studio) могут сначала предложить добавить `?` или `!`,
вместо предложения импорта. Чтобы обойти это:

1. Временно добавьте `!` в выражение: `maybeNull!.debang('...')`. Это изменит
   ошибку на неразрешённую ссылку, что вызовет предложение импорта.
2. Используйте быстрое исправление (например, Ctrl+. в VS Code) для добавления
   импорта.
3. Удалите `!` после добавления импорта.

Альтернативно, вручную добавьте импорт в начало файла:
`import 'package:debang/debang.dart';`. Это полностью обходит проблему с быстрым
исправлением.
